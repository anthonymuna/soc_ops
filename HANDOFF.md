...
SSH command:
```bash
ssh -i keys/syndicate4.pem -o StrictHostKeyChecking=no ubuntu@<server-ip>
```
...
All ports bind to host `127.0.0.1` — accessible securely via SSH tunnel (`./deploy.sh --tunnel`).
...
Base URL: `http://localhost:8000` (via tunnel)
...
1. **Verify all 8 containers are running:**
   ```bash
   ssh -i keys/syndicate4.pem ubuntu@<server-ip> 'sudo docker ps'
   ```

2. **Trigger model training (NSL-KDD download + RF train):**
   ```bash
   # Run via SSH tunnel or locally on server
   curl -X POST http://localhost:8000/train
   ```
   First train downloads ~4MB NSL-KDD, trains RF (~30s), saves model.

3. **Verify frontend loads:** `http://localhost:3000` (via tunnel)

4. **Verify Kibana loads:** `http://localhost:5601` (via tunnel)
...
## Access Points (accessed via tunnel: `./deploy.sh --tunnel`)

| Service | URL |
|---|---|
| **SOC Dashboard (React)** | http://localhost:3000 |
